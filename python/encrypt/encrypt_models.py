...
ignore_file_ext = [".json", ".cfg", ".cdnn", ".mean", ".a", ".b", ".config", ".c", ".jpg", ""]
...
def encrypt(encrypt_cmd, input_file_path, output_file_path):
    ...
    if status != 0:
        print "Encrypt model %s failed, error msg:%s" % (input_file_path, returnmsg)
    else:
...
def start(input_folder_path, output_folder_path, encrypt_cmd, recursive=True):
    os.system("chmod 777 %s" % encrypt_cmd)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-c", dest="encrypt_cmd", default="watch_dog_encryptor", help="the command to encrypt the model")
    parser.add_argument("-m", dest="model_folder_name", default="model",
                        help="the folder name contains unencryption model")

    args = parser.parse_args()
    input_folder = args.input_folder or "%s/%s" % (os.getcwd(), "model")
